...
export default function ComplianceTable({ 
  ...
  const filterTabs = [
    ...
    { id: 'Compliant', label: 'Compliant', count: requirements.filter(r => r.status === 'Compliant').length },
    { id: 'Partial', label: 'Partial', count: requirements.filter(r => r.status === 'Partial').length },
    { id: 'Missing', label: 'Missing', count: requirements.filter(r => r.status === 'Missing').length },
    { id: 'High Risk', label: 'High Risk', count: requirements.filter(r => r.risk === 'High').length },
  ];
  ...
  const filteredRequirements = useMemo(() => {
    return requirements.filter(item => {
      ...
      if (activeFilter === 'Compliant') matchesFilter = item.status === 'Compliant';
      else if (activeFilter === 'Partial') matchesFilter = item.status === 'Partial';
      else if (activeFilter === 'Missing') matchesFilter = item.status === 'Missing';
      else if (activeFilter === 'High Risk') matchesFilter = item.risk === 'High';

      // Search query condition
      const matchesSearch = 
        item.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
        item.clause.toLowerCase().includes(searchQuery.toLowerCase()) ||
        item.evidenceFound.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/90 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 px-2.5 py-0.5 rounded-md bg-indigo-50 text-indigo-700 text-xs font-bold mb-1">
            ...
            <span>Master Compliance Register</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">
            All Requirements
          </h1>
          <p className="text-sm text-slate-600 mt-1">
            Complete traceability matrix comparing Industrial Safety Standard 2026 to plant audit records.
          </p>
        </div>

        <button
          onClick={() => onNavigate('chat')}
          className="px-4 py-2 rounded-xl bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-bold border border-indigo-200 transition flex items-center gap-2"
        >
      ...
      <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-xs flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
        ...
        <div className="relative min-w-[240px]">
          ...
          <input
            type="text"
            placeholder="Search requirement, clause, evidence..."
            value={searchQuery}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/80 border-b border-slate-200/80 text-[11px] font-bold uppercase tracking-wider text-slate-500">
                ...
                <th className="py-3.5 px-4 sm:px-6">Requirement</th>
                <th className="py-3.5 px-4">Status</th>
                <th className="py-3.5 px-4">Risk</th>
                <th className="py-3.5 px-4 sm:px-6">Evidence / Findings</th>
                      ...
                      <td className="py-4 px-4 whitespace-nowrap">
                        {isResolved ? (
                          ...
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-red-50 text-red-700 border border-red-200">
                            <AlertOctagon className="w-3.5 h-3.5" /> Missing
                          </span>
                        )}
                      </td>
